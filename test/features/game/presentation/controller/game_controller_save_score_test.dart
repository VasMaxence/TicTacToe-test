...
void main() {
  ...
  setUp(() {
    ...
    controller = GameController(playMove: mockPlayMove, difficulty: Difficulty.easy, saveScoreUseCase: mockSaveScore);
  });

  setUpAll(() {
    registerFallbackValue(GameState.initial(Difficulty.easy));
    registerFallbackValue(Score(board: const [], boardSize: 3, winningLine: null, difficulty: Difficulty.easy, playedAt: DateTime.now()));
  });

  test('should call saveScore when game is over after a move (win)', () async {
    final initialState = GameState.initial(Difficulty.easy);
